  ...
  Future<void> _validarCupon() async {
    ...
    try {
      final rs = await controladorPromocion.validarCupon(
          _cupon, widget.paymentIntentId);

      if (rs["success"] == true) {
        setState(() {
          _nuevoMonto =
              double.tryParse(rs["nuevo_amount"].toString());
    ...
    try {
      await Stripe.instance.initPaymentSheet(
        paymentSheetParameters: SetupPaymentSheetParameters(
          ...
          merchantDisplayName: 'Servirentamx',
      ...
      if (!mounted) return;
      Funciones().mostrarMensaje("ok", 'Pago realizado con éxito');
      Navigator.pop(context);
    } on StripeException catch (e) {
      Funciones().mostrarMensaje("error",
          'Error Stripe: ${e.error.localizedMessage}');
    } catch (e) {
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed:
                        _aplicandoCupon||_promoData!=null ? null : () => _validarCupon(),
