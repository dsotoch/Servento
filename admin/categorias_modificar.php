...
try {
    ...
    if ($tipo === 'subcategoria' && empty($categoria_padre_id)) {
        echo json_encode(['status' => 'error', 'mensaje' => 'Debe seleccionar una categoría padre.']);
        exit;
    }
    ...
    if ($tipo === 'categoria') {
        ...
        $sql = "UPDATE subcategorias 
                SET categoria_id = :categoria_id, nombre = :nombre, descripcion = :descripcion, estado = :estado 
                WHERE id = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([
        ...
        $mensaje = "Subcategoría actualizada correctamente bajo la categoría '$nombrePadre'.";
    }
